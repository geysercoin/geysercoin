...
const string strMessageMagic = "GeyserCoin Signed Message:\n";
...
int64_t GetProofOfStakeReward(const CBlockIndex* pindexPrev, int64_t nCoinAge, int64_t nFees)
{
     ...
     if(pindexBest->nHeight < GetLastPowBlock())
    {
        ...
        else if(pindexBest->nHeight >= GetLastPowBlock() && pindexBest->nHeight <= 564480)
    {
          nSubsidy = nCoinAge * COIN_YEAR_REWARD * 33 / (365 * 33 + 8);  //default 100% yearly
    }
		else if(pindexBest->nHeight >= GetLastPowBlock() && pindexBest->nHeight <= 604800)
    {
...
{
    RenameThread("geysercoin-loadblk");
